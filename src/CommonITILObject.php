...
abstract class CommonITILObject extends CommonDBTM
{
   ...
   // STATUS статусы
    const INCOMING      = 1; // new новый
    const ASSIGNED      = 2; // assign назначен
    const PLANNED       = 3; // plan запланирован
    const WAITING       = 4; // waiting ожидание
    const SOLVED        = 5; // solved решен
    const CLOSED        = 6; // closed закрыт
    const ACCEPTED      = 7; // accepted принят в обработку
    const OBSERVED      = 8; // observe наблюдение
    const EVALUATION    = 9; // evaluation оценка
...
//Сбрасывает данные об акторах (пользователях, группах, поставщиках), чтобы при следующем обращении они загрузились заново.
...
//Загружают связанных пользователей, группы и поставщиков.
//Что делают: Используют соответствующие классы-связки для получения акторов текущего объекта.
    /**
     * Load linked users
     *
     * @return void
...
//Вызывает методы загрузки пользователей, групп и поставщиков, предварительно очищая кэш.
    /**
     * @since 0.84
     **/
...
//Очищает кэш лениво загруженных акторов.
    /**
...
//При обращении к users, groups, suppliers автоматически загружают данные, если они еще не загружены.
    /**
...
//Возвращает общее количество акторов объекта.
//Что делает: Суммирует количество групп, пользователей и поставщиков.
    /**
...
//Возвращает акторов заданного типа (заявитель, исполнитель, наблюдатель).
//Что делает: Учитывает различные источники данных (шаблоны, значения по умолчанию, уже сохраненные данные) и возвращает форматированный список акторов.
    /**
...
//Назначение: Восстанавливает введенные пользователем данные и устанавливает значения по умолчанию.
//Что делает: Используется при отображении формы для обеспечения сохранности введенных данных между запросами.
    /**
...
//Отображает форму создания/редактирования ITIL-объекта.
//Что делает:
//Проверяет права доступа.
//Восстанавливает введенные данные и значения по умолчанию.
//Загружает соответствующий шаблон.
//Отображает форму с использованием шаблонизатора.
    /**
     * @param $ID
     * @param $options   array
     **/
...
//Устанавливает предопределенные поля из шаблона.
//Что делает:
//Обрабатывает поля из шаблона.
//Обновляет переданные опции ($options) предопределенными значениями.
//Учитывает особые случаи (например, преобразование тикета в проблему или изменение).
    /**
...
//Определяет возможные организации для заявителей.
//Что делает: Анализирует переданные данные и возвращает список организаций, к которым принадлежат заявители.
    /**
...
//Загружает объект из БД вместе с дополнительными данными.
//Что делает: Вызывает getFromDB() и затем getAdditionalDatas().
    /**
...
//Загружает дополнительные данные объекта (пустой метод, может быть переопределен в дочерних классах).
    public function getAdditionalDatas()
    {
    }

//Проверяют различные права доступа к объекту.
//Что делают: Возвращают true или false в зависимости от прав текущего пользователя и состояния объекта.
    /**
     * Can manage actors
     *
     * @return boolean
...
//Проверяет, нужно ли переоткрывать объект.
//Что делает: Анализирует роли текущего пользователя и статус объекта чтобы определить, может ли пользователь переоткрыть его.
    /**
...
//Проверяет, является ли пользователь валидатором для этого объекта.
//Что делает: Ищет записи о валидации, связанные с текущим объектом и пользователем.
    /**
...
//Проверяют, связан ли конкретный пользователь/группа/поставщик с объектом в определенной роли.
//Что делают: Ищут соответствующие записи в свойствах объекта.
    /**
...
// Возвращают акторов определенного типа.
//Что делают: Возвращают соответствующие части свойств users, groups, suppliers.
    /**
...
//Возвращает всех пользователей, включая тех, кто входит в группы-акторы.
//Что делает: Объединяет пользователей-акторов и пользователей из групп-акторов.
    /**
...
//Подсчитывают количество акторов определенного типа или всех акторов.
//Что делают: Возвращают количество записей в соответствующих свойствах.
    /**
...
//Проверяет, связана ли хотя бы одна из переданных групп с объектом в определенной роли.
//Что делает: Ищет совпадения ID групп в свойствах объекта.
    /**
...
//Возвращает актора по умолчанию для определенной роли.
//Что делает: Например, для исполнителя возвращает текущего пользователя, если у него есть право "own_ticket".
    /**
...
//Возвращает право доступа для поиска акторов по умолчанию.
//Что делает: Для разных ролей возвращает разные права доступа.
    /**
     * Get Default actor when creating the object
     *
     * @param integer $type type to search (see constants)
...
//Подсчитывает количество активных объектов для определенного актора.
//Что делает: Выполняет SQL-запрос для подсчета объектов, которые не удалены и не находятся в закрытом или решенном статусе.
    /**
...
//Специализированные методы для подсчета активных объектов для разных типов акторов.
//Что делают: Вызывают countActiveObjectsFor() с соответствующими параметрами.
    /**
...
//Очищает связанные данные при удалении объекта.
//Что делает: Удаляет записи из связанных таблиц (проекты, followups, решения, акторы).
...
//Обрабатывает обязательные поля из шаблона при обновлении объекта.
//Что делает:
//Проверяет права пользователя.
//Проверяет обязательные поля из шаблона.
//Возвращает подготовленные данные или false при ошибке.
    /**
...
//Подготавливает входные данные перед обновлением объекта.
//Что делает:
//Проверяет согласованность полей.
//Обрабатывает шаблонные поля.
//Добавляет/удаляет документы.
//Обрабатывает акторов (заявителей, исполнителей, наблюдателей).
//Обновляет даты и статусы в зависимости от изменений.
...
//Выполняется после обновления элемента в базе данных.
//Что делает:
//Обрабатывает файлы (изображения и документы) в тексте и загруженные документы
//Обновляет акторов (пользователей, группы, поставщиков)
//Обрабатывает шаблоны задач, followup'ов и решений
//Управляет запросами валидации
//Вызывает родительский метод post_updateItem
...
//Выполняется перед обновлением элемента в базе данных.
//Что делает:
//Перезагружает акторов
//Проверяет изменения дат (игнорирует изменения только в секундах)
//Устанавливает даты решения и закрытия в зависимости от изменения статуса
//Проверяет корректность дат (например, дата решения не может быть раньше даты создания)
//Управляет временем ожидания и пересчитывает сроки (SLA/OLA) при изменении статуса
//Обновляет статистические поля (например, solve_delay_stat)
//Обрабатывает повторное открытие объекта (отмечает существующие решения как отклоненные, удаляет опрос удовлетворенности)
//Удаляет обновление date_mod, если других изменений нет
...
//Подготавливает входные данные перед добавлением объекта.
/*  то делает:
Проверяет согласованность полей
Преобразует входные данные акторов
Устанавливает значения по умолчанию для urgency, impact, priority
Устанавливает получателя (users_id_recipient) на основе текущего пользователя или заявителя
Генерирует название (name) из содержания, если оно пустое
Устанавливает значения по умолчанию для некоторых полей (entities_id, itilcategories_id)
Вычисляет значения по умолчанию для дат (date, solvedate, closedate) в зависимости от статуса
Проверяет обязательные поля из шаблона
*/
...
Что делает:
Проверяет, что поле 'date' имеет правильный формат (если присутствует и не пустое)
*/
    /**
...
//Вычисляет значения по умолчанию для добавления объекта.
//Что делает:
//Устанавливает статус по умолчанию (INCOMING)
//Устанавливает текущее время для даты создания, если не указано
//Устанавливает даты решения и закрытия в зависимости от статуса
//Устанавливает дату начала ожидания, если статус WAITING
    /**
...
/*Выполняется после добавления элемента в базу данных.
Что делает:
Обрабатывает файлы (изображения и документы)
Добавляет документы по умолчанию из шаблона
Обновляет акторов
Обрабатывает шаблоны задач, followup'ов и решений
Управляет запросами валидации
Вызывает родительский метод post_addItem
*/
...
//Выполняется после клонирования элемента.
//Что делает: Обновляет поля users_id_lastupdater и status в клонированном объекте
    /**
...
//Возвращает отношения, которые должны быть клонированы.
//Что делает: Возвращает классы связей с акторами (пользователями, группами, поставщиками)
...
/*
Вычисляет приоритет на основе urgency и impact.
Что делает
Использует матрицу приоритетов из конфигурации GLPI или вычисляет среднее
*/
    /**
     * Compute Priority
     *
...
//Отображает выпадающий список приоритетов.
//Что делает:
//Формирует список приоритетов на основе конфигурации и переданных опций
//Учитывает маски urgency и impact для фильтрации доступных приоритетов
    /**
...
//Назначение: Возвращает текстовое название приоритета по его числовому значению.
    /**
...
//Отображает выпадающий список urgency.
//Что делает:
//Формирует список urgency на основе конфигурации и переданных опций
    /**
...
//Возвращает текстовое название urgency по его числовому значению.
    /**
...
//Отображает выпадающий список impact.
//Что делает:Формирует список impact на основе конфигурации и переданных опций
    /**
...
//Возвращает текстовое название impact по его числовому значению.
    /**
...
//Возвращает массив всех статусов объекта.
//Примечание: Должен быть переопределен в дочерних классах
    /**
...
//Возвращает массив статусов, считающихся закрытыми.
//Примечание: Должен быть переопределен в дочерних классах
    /**
...
//Возвращает массив статусов, считающихся решенными.
//Примечание: Должен быть переопределен в дочерних классах
    /**
...
//Возвращает массив статусов, которые не являются решенными или закрытыми.
    /**
...
//Возвращает массив статусов, считающихся новыми.
//Примечание: Должен быть переопределен в дочерних классах
    /**
...
//Возвращает массив статусов, считающихся процессами.
...
//Возвращает массив статусов процессов.
    /**
...
//Проверяет, разрешен ли переход от старого статуса к новому на основе матрицы прав.
    /**
...
//Проверяет, не находится ли объект в решенном или закрытом статусе.
    /**
...
//Проверяет, находится ли объект в решенном статусе (опционально включая закрытые).
    /**
...
//Проверяет, находится ли объект в закрытом статусе.
    /**
...
//Возвращает массив статусов, в которые можно перейти из текущего статуса.
    /**
...
//Проверяет, существует ли указанный статус для данного объекта.
    /**
...
//Отображает выпадающий список статусов.
    /**
...
//Возвращает текстовое название статуса по его числовому значению.
    /**
...
//Возвращает строковое представление типа актора (requester, observer, assign).
    /**
...
//Возвращает отображаемое значение для указанного поля (статус, urgency, impact, priority, global_validation).
    /**
...
//Возвращает элемент управления для выбора значения указанного поля (статус, urgency, impact, priority, global_validation).
    /**
     * @since 0.84
     *
     * @param $field
...
//Отображает подформу для массовых действий.
//Что делает:
//Для действия 'add_task' отображает форму добавления задачи
//Для действия 'add_actor' отображает выбор типа актора и динамически загружает поля для выбора актора
//Для действия 'update_notif' отображает выбор yes/no для уведомлений
    /**
     * @since 0.85
     *
...
/*
Обрабатывает массовые действия для одного типа элемента.
Что делает:
Для действия 'add_actor' добавляет акторов к выбранным объектам
Для действия 'update_notif' обновляет настройки уведомлений для акторов выбранных объектов
Для действия 'add_task' добавляет задачу к выбранным объектам
*/
    /**
     * @since 0.85
     *
...
/*
Возвращает основные опции поиска для ITIL-объектов.
Что делает: Создает массив с полями для поиска, включая:
ID, название, описание, статус, urgency, impact, priority
Даты: открытия, закрытия, решения, последнего обновления
Категорию, организацию, длительность
Документы, причины ожидания, местоположение, проекты
*/
    /**
     * @since 0.85
     **/
...
/*
Возвращает опции поиска, связанные с решением.
Что делает: Включает:
Тип решения
Содержание решения
Статус решения (любой или последний) */
    /**
     * @since 0.85
     **/
...
/*
Возвращает опции поиска для статистических данных.
Что делает: Включает:
Время решения
Время закрытия
Время ожидания
...
/*
Возвращает опции поиска для акторов (заявители, наблюдатели, исполнители).
Что делает: Включает:
Пользователей-заявителей, группы-заявители, писателя
Наблюдателей (пользователи и группы)
Исполнителей (пользователи, поставщики, группы)
Настройки уведомлений (email для followup)*/
...
/*
Генерирует SQL-выражение для вычисления нарушения SLA/OLA.
Что делает: Возвращает условие для проверки, превышено ли время:
time_to_own / internal_time_to_own
time_to_resolve / internal_time_to_resolve
*/
...
/*
Возвращает HTML-код иконки для статуса.
Что делает: Использует getStatusClass для определения класса иконки и добавляет всплывающую подсказку.*/
    /**
     * Get status icon
     *
     * @since 9.3
...
/*
Возвращает CSS-класс для иконки статуса.
Что делает: Для каждого статуса возвращает соответствующий класс FontAwesome и дополнительный класс для цвета.*/
    /**
     * Get status class
     *
     * @since 9.3
...
/*
Возвращает ключ (строковый идентификатор) для статуса.
Что делает: Сопоставляет числовой статус с строковым ключом (например, 1 -> 'new').*/
    /**
     * Get status key
     *
     * @since 9.3
...
/*
Показывает форму добавления актора (пользователя, группы, поставщика).
Что делает:
Отображает dropdown для выбора типа актора (user, group, supplier)
Динамически загружает соответствующий dropdown для выбора конкретного актора
Учитывает скрытые поля и права доступа*/
    /**
     * show actor add div
     *
...
/*
Показывает форму добавления пользователя при создании объекта.
Что делает:
Отображает dropdown пользователей с учетом прав доступа
Показывает количество активных объектов для пользователя (для заявителей)
Обновляет информацию об уведомлениях и количестве назначений через AJAX*/
    /**
...
//Форматирует время действия в читаемый вид.
    /**
     * @param $actiontime
     **/
...
//Возвращает имя назначенного объекта (пользователя, группы, поставщика).
//Что делает: В зависимости от типа объекта возвращает имя с ссылкой или без.
    /**
     * @param $ID
     * @param $itemtype
...
//Показывает форму массового добавления решения.
//Что делает: Вызывает метод showForm класса ITILSolution с параметрами для массового действия.
    /**
...
//Обновляет дату изменения и последнего редактора.
//Что делает: Устанавливает текущее время и ID пользователя (если не cron) в поле date_mod и users_id_lastupdater.
    /**
...
//Обновляет общее время действий на основе задач.
//Что делает: Суммирует время всех задач, связанных с объектом, и обновляет поле actiontime.
    /**
...
//Возвращает все типы объектов, которые могут быть связаны с ITIL-объектом.
//Что делает: Объединяет типы из конфигурации и плагинов, проверяя права доступа.
    /**
...
//Проверяет, можно ли назначить ITIL-объект на указанный тип.
//Что делает: Проверяет наличие типа в активном профиле пользователя.
    /**
...
//Вычисляет время решения с учетом календаря.
//Что делает: Использует календарь для вычисления активного времени между датой создания и решения, вычитая время ожидания.
    /**
...
//Вычисляет время закрытия с учетом календаря.
//Что делает: Аналогично computeSolveDelayStat, но для даты закрытия.
    /**
...
//Показывает статистику объекта (даты и времена).
//Что делает: Вызывает showStatsDates и showStatsTimes, а также хуки плагинов.
...
//Показывает таблицу с датами объекта.
//Что делает: Отображает даты открытия, решения, закрытия и время до решения.
...
//Показывает таблицу с временными показателями.
//Что делает: Отображает время принятия, решения, закрытия и ожидания.
...
//Возвращает авторов (заявителей) объектов между двумя датами.
//Что делает: Выполняет запрос к базе данных для получения distinct пользователей-заявителей.
...
//Возвращает получателей объектов между двумя датами.
//Что делает: Выполняет запрос к базе данных для получения distinct получателей (users_id_recipient).
...
//Возвращает группы-заявители объектов между двумя датами.
//Что делает: Выполняет запрос к базе данных для получения distinct групп-заявителей.
...
//Возвращает должности или категории пользователей-заявителей между двумя датами.
//Что делает: В зависимости от параметра title возвращает должности или категории пользователей.
    /** Get recipient of itil object between 2 dates
     *
     * @param string  $date1 begin date
     * @param string  $date2 end date
...
//Возвращает приоритеты объектов между двумя датами.
//Что делает: Выполняет запрос к базе данных для получения distinct приоритетов.
    /**
...
//Возвращает urgency объектов между двумя датами.
//Что делает: Выполняет запрос к базе данных для получения distinct urgency.
    /**
...
//Возвращает impact объектов между двумя датами.
//Что делает: Выполняет запрос к базе данных для получения distinct impact.
    /**
...
//Возвращает типы запросов объектов между двумя датами.
//Что делает: Выполняет запрос к базе данных для получения distinct requesttypes_id.
    /**
...
//Возвращает типы решений, использованные в объектах между двумя датами.
//Что делает: Выполняет запрос к базе данных для получения distinct solutiontypes_id из решений, связанных с объектами.
    /**
...
//Возвращает техников (исполнителей), назначенных на объекты между двумя датами.
//Что делает: Выполняет запрос к базе данных для получения distinct пользователей-исполнителей.
...
//Возвращает техников, которые выполняли задачи по объектам между двумя датами.
//Что делает: Выполняет запрос к базе данных для получения distinct пользователей, которые являются исполнителями задач и имеют право ticket -> OWN.
...
//Возвращает поставщиков, назначенных на объекты между двумя датами.
//Что делает: Выполняет запрос к базе данных для получения distinct поставщиков-исполнителей.
...
//Возвращает группы, назначенные как исполнители на объекты между двумя датами.
//Что делает: Выполняет запрос к базе данных для получения distinct групп-исполнителей.
...
//Отображает краткую информацию об объекте в виде строки таблицы.
//Что делает:
//Загружает объект по ID
//Форматирует и выводит информацию: ID, название, статус, даты, приоритет, заявителей, исполнителей, связанные элементы, категорию, планировку
//Учитывает различные параметры отображения (массовые действия, статистика и т.д.)
    /**
...
//Отображает заголовок таблицы для списка объектов.
//Что делает: Выводит строку с названиями колонок: ID, название, статус, даты, приоритет, заявители, исполнители и т.д.
    /**
...
//Возвращает ID календаря, используемого для объекта.
//Что делает: Использует конфигурацию организации для определения календаря.
    /**
...
//Определяет позицию элемента на временной шкале (слева или справа).
//Что делает: На основе роли пользователя (заявитель, наблюдатель, исполнитель) возвращает позицию для отображения.
    /**
...
//Возвращает типы элементов, которые могут отображаться на временной шкале.
//Что делает: Формирует массив с информацией о followup'ах, задачах, решениях, документах и валидациях, включая права доступа.
...
//Возвращает HTML-код действий временной шкалы, предоставляемых плагинами.
//Что делает: Вызывает хук плагинов и возвращает сгенерированный HTML.
    /**
...
//Возвращает все элементы временной шкалы для объекта.
//Что делает: Собирает followup'ы, задачи, решения, документы, валидации и логи, учитывая права доступа и параметры.
    /**
...
//Отображает подформу для добавления связанного элемента.
//Что делает: В зависимости от типа элемента вызывает соответствующий метод отображения формы.
    /**
     * @since 9.4.0
     *
...
//Отображает форму редактирования описания объекта.
//Что делает: Использует шаблон для отображения формы с учетом прав доступа.
...
//Возвращает список акторов объекта (пользователей и их роли).
//Что делает: Выполняет запрос к базе данных для получения пользователей и их ролей (заявитель, наблюдатель, исполнитель).
    /**
...
//Возвращает количество followup'ов объекта.
//Что делает: Выполняет запрос к базе данных для подсчета followup'ов, учитывая приватность.
    /**
...
//Возвращает количество задач объекта.
//Что делает: Выполняет запрос к базе данных для подсчета задач, учитывая приватность.
    /**
...
//Проверяет, заблокирован ли расчет времени принятия в работу.
//Что делает: Проверяет наличие флага _do_not_compute_takeintoaccount во входных данных.
    /**
...
//Проверяет, заблокирован ли расчет статуса.
//Что делает: Проверяет наличие флага _do_not_compute_status во входных данных.
    /**
...
//Добавляет вкладку по умолчанию в форму объекта.
//Что делает: Добавляет вкладку с названием типа объекта и количеством элементов временной шкалы.
...
//Возвращает дополнительные опции меню для объекта.
//Что делает: Возвращает массив с ссылками на шаблоны объекта, если есть права на их просмотр.
...
//Возвращает дополнительные ссылки меню для объекта.
//Что делает: Возвращает массив с ссылками на шаблоны и канбан-доску.
    /**
...
//Возвращает шаблон, который следует использовать для объекта.
//Что делает: Определяет шаблон на основе принудительного шаблона, категории, типа, профиля пользователя и организации.
    /**
     * Get template to use
...
//Возвращает название поля шаблона в зависимости от типа.
//Что делает: Формирует название поля для шаблона (например, tickettemplates_id_incident для инцидентов).
    /**
...
//Возвращает значения по умолчанию для объекта (абстрактный метод).
//Что делает: Должен быть реализован в дочерних классах.
    /**
     * @since 9.5.0
     *
...
//Возвращает класс шаблона для объекта.
//Что делает: Возвращает имя класса шаблона (например, TicketTemplate для тикетов).
    /**
...
//Возвращает название поля формы для шаблона.
//Что делает: Возвращает строку с названием поля (например, _tickettemplate).
    /**
