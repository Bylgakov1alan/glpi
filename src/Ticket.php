...
{
   // From CommonDBTM
   //указывает нужно ли записывать историю изменений
    public $dohistory                   = true;
    //список классов которым передеатеся информация о сущности
    protected static $forward_entity_to = ['TicketValidation', 'TicketCost'];

   // From CommonITIL
   //классы связывающие билеты с пользователями группами и поставщиками
    ...
    // при создании пользователя автоматически устанавливается его сущность
    protected $userentity_oncreate      = true;

    // константы поля используемые для определения матрицы приоритетов, срочности, влияния и статусов
   ...
   /// Is a hardware found in getHardwareData / getFromDBwithData : hardware link to the job
    ...
    // константы типа заявок
   // Request type
    const INCIDENT_TYPE = 1;
   // Demand type
    const DEMAND_TYPE   = 2;

    // константы прав доступа к билетам
    const READMY           =      1;
    const READALL          =   1024;
...
//возвращает список запрещенных массовых действий для текущего пользователя на основе его прав
...
//Возвращает символ используемый в меню для ссылки на создание нового билета
...
// добавляет дополнитьльное меню если пользователь может создавать заявки
...
//возвращает дополнительные ссылки в меню например для проверки одобренных заявок
    public static function getAdditionalMenuLinks()
    {
        ...
        if (Session::haveRightsOr('ticketvalidation', TicketValidation::getValidateRights())) {
            ...
            $pic_validate = '<i class="ti ti-eye-check" title="' . __s('Tickets waiting for your approval') . '"></i>
            <span class="d-none d-xxl-block">' . __s('Tickets waiting for your approval') . '</span>';
...
//Различные методы, определяющие, может ли текущий пользователь выполнять определенные действия с билетом (назначение, обновление, просмотр и т.д.) 
// методы для проверки прав доступа к чему либо
...
// c 155 до 388 строки методы которые проверяют наличие прав на выполнение каких либо действий пользователем



// управляют добавлением, удалением и получением данных для SLA и OLA
...
// работа с SLA и OLA с 392 строки до 538 строки


// может ли пользователь создавать новую заявку
    /**
...
//может ли пользователь обновлять билет
    /**
...
//начинается на 556 строке заканчиваются на 608 строке

//определяет является ли пользователь создателем-владельцем билета
    /**
...
//проверяет возможность переоткрытия заявки
    /**
...
//возвращают информацию о пользователе по умолчанию, который будет назначен на заявку
    /**
...
// начинается на 678 заканчивается на 719 строках

//выполняется перед тем как удалиться заявка
...
//определяет название вкладок для отображения информации о билетах в других сущностях
...
//отображает содержимое вкладки для конкректной сущности
...
// определяет список вкладок которые будут отображаться при просмотре билета
...
// получает дополнительную информацию о связанном оборудовании
    /**
...
//чистит базу данных от связанных записей при удалении билета
...
//подготавливает входящие данные перед обновлением обьекта
...
//функции управляют влиянием SLA и OLA на билет при его обновлении
    /**
...
//начинается на 1335 заканчивается на 1517 строке


//функции управляют уровнями SLA и OLA 
    /**
...
// начинается на 1520 заканчиваются на 1576 строке

//Эта функция выполняется перед обновлением записи в базе данных. 
// Она проверяет, нужно ли установить дату принятия заявки в работу (takeintoaccountdate) 
// и вычисляет задержку принятия в статистике. Это используется для отслеживания времени, 
// которое прошло с момента создания заявки до момента ее принятия к работе.
...
//Вычисляет задержку между датой создания заявки и датой ее принятия в работу. 
// Использует календарь рабочего времени, если он задан, или просто считает разницу в секундах.
    /**
...
//Выполняется после обновления билета. 
// Обрабатывает специальные случаи, такие как автоматическое закрытие дубликатов при решении или закрытии основного билета.
...
//Подготавливает данные перед добавлением нового билета в базу данных
...
//Выполняется после успешного создания нового билета.
...
//Получает активные или решенные билеты для конкретного оборудования за последние N дней
    /**
...
//Подсчитывает количество активных билетов для конкретного оборудования
    /**
...
//Получает активные билеты для конкретного оборудования с фильтрацией по типу
    /**
...
//Подсчитывает количество решенных билетов для оборудования за последние N дней
    /**
...
//Обновляет дату модификации билета и вычисляет статистику принятия в работу.
    /**
...
//Проверяет, может ли пользователь добавлять элементы к билету
    /**
...
//Проверяет, может ли конкретный пользователь добавлять комментарии к билету
    /**
...
//Проверяет, может ли текущий пользователь создать билет от имени другого пользователя
    /**
...
//Возвращает стандартный запрос для поиска билетов
    /**
...
//Определяет специальные действия для массовой обработки билетов
    /**
...
//Отображает дополнительную форму для выбора параметров при массовых действиях
...
//Обрабатывает массовые действия для одного типа элементов
...
//Эта функция определяет все доступные поля для поиска и фильтрации в системе GLPI при работе с билетами.
...
// Преобразует значение поля для отображения в определенном формате
...
//Создает элемент формы для выбора значения определенного поля
    ...
    //Создает выпадающий список типов тикетов
    /**
...
//Возвращает массив доступных типов тикетов
    /**
     * Get ticket types
     *
...
//Возвращает читаемое название типа тикета по его ID
    /**
...
//Возвращает массив всех статусов тикетов////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    /**
...
//Возвращает массив статусов, считающихся находящимися в обработке
    /**
...
//Вычисляет общую стоимость владения (TCO) для элемента
    /**
...
//Отображает форму создания тикета в службе поддержки
    /**
...
//Отображает форму добавления наблюдателя в тикет
    /**
...
//Возвращает значения по умолчанию для создания тикета
...
//Отображает основную форму создания/редактирования тикета
...
//Отображает централизованный список тикетов
    /**
     * @param integer $start
...
//Формирует критерии для подсчета тикетов в центральном представлении
    /**
...
//Отображает количество тикетов по статусам в центральном представлении
    /**
     * Get tickets count
     *
     * @param boolean $foruser  Only for current login user as requester or observer (false by default)
...
//Отображает список новых тикетов в центральном представлении
...
//Отображает тикеты для указанного элемента
//Также отображает тикеты связанных элементов
    /**
...
//Отображает краткую информацию о тикете
    /**
     * @param $ID
...
//Возвращает общие критерии для запросов, связанных с тикетами
...
//Отображает предварительное действие назначения для тикета
    /**
     * @param $output
     **/
...
//Возвращает информацию о задачах крона, связанных с тикетами
    /**
...
//Выполняет автоматическое закрытие тикетов по расписанию
    /**
...
//Cron-задача для отправки уведомлений о старых незакрытых билетах
    /**
...
//Автоматическое удаление старых закрытых заявок.
    /**
...
//Отладочная информация для объекта заявки.
    /**
...
//Определение прав доступа к заявкам.
    /**
     * @since 0.85
     *
...
//Конвертация изображений в HTML-контенте для корректного отображения в заявках.
    /**
...
//Получение календаря для расчета временных метрик
    /**
...
//Кастомизация отображения полей в формах
    /**
...
//Визуализация временной шкалы заявки
...
//Подготовка данных для бизнес-правил
    /**
...
//    Построение SQL-условий для ограничения видимости заявок
    /**
...
//Возвращает список запрещенных одиночных массовых действий
...
//Возвращает список разрешенных одиночных массовых действий
    ...
    /**
    * Объедините один или несколько тикетов с другим существующим тикетом.
     * При желании в объединенный тикет можно скопировать такие подпункты, как последующие действия, документы и задачи.
     * Если объединить тикет не удается, процесс продолжается до следующего тикета.
     * @param int $merge_target_id - Идентификатор билета, в который будут объединены другие билеты
     * @param array $ticket_ids - массив идентификаторов билетов, которые будут объединены в билет с идентификатором $merge_target_id
     * массив @param $params - массив параметров для слияния заявок.
     * linktypes - массив типов элементов, которые будут дублироваться в ticket $merge_target_id.
     * По умолчанию никакие подпункты не копируются. В настоящее время поддерживаются следующие типы ссылок: ITILFollowup, Document и TicketTask.
     * full_transaction - логическое значение, указывающее, должно ли полное слияние завершиться успешно или разрешены частичные слияния.
     * По умолчанию должно завершиться полное слияние. В случае сбоя вся база данных будет работать
     * Объединяет несколько заявок в одну целевую заявку 
    */
...
//Формирует критерии для фильтрации заявок на основе прав текущего пользователя
    /**
...
//Возвращает иконку для заявки.
    public static function getIcon()
    {
        return "ti ti-alert-circle";
    }

//Возвращает класс для связи элементов с заявкой.  
    public static function getItemLinkClass(): string
    {
        return Item_Ticket::class;
    }

//Возвращает класс для задач заявки.
    public static function getTaskClass()
    {
        return TicketTask::class;
    }

//Возвращает класс параметров шаблонов содержимого для заявок.
    public static function getContentTemplatesParametersClass(): string
    {
        return TicketParameters::class;
    }

//Формирует ограничения для выборки заявок, связанных с определенным объектом (например, пользователем, группой).
...
//Формирует критерии для поиска заявок, связанных с определенным объектом
