...
class Provider
{
    ...
    // Добавил что бы появился новый виджет в панели с кол-вом заявок где статус равен эскалированно на вендера
    public static function nbTicketsEscalated(array $params = []): array
        {
            $default_params = [
                'label'         => __("Escalated to vendor"),
    ...
    public static function nbTicketsGeneric(
        ...
        switch ($case) {
           ...
           // Statuses speciale cases (no break)
            ...
            case 'escalated':                   // Добавил что бы появился новый виджет в панели с кол-вом заявок где статус равен эскалированно на вендера
                if (!$skip) {
                    $status = Ticket::ESCALATED;
                    $params['icon']  = "fas fa-external-link-alt";
                    $params['label'] = __("Escalated to vendor");
                    $skip = true;
                }
               //no break
            case 'assigned':
